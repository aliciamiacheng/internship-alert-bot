...
def scrape_company(company, cfg):
    ...
    try:
        r = requests.get(company["careers_url"], headers=headers,
                         timeout=cfg["settings"]["request_timeout_seconds"])
        r.raise_for_status()
    except Exception as e:
        print(f"[WARN] {company['company']}: {e}")
        return []
    ...
    for a in soup.find_all("a", href=True):
        ...
        dedupe.add(key)
        jobs.append({"company":company["company"],"title":title,"url":url,
                     "categories":categories})
    return jobs

def format_alert(jobs):
    lines=["# New internship matches",""]
    for j in jobs:
        cats=", ".join(j["categories"])
        lines += [f"## {j['company']} — {j['title']}", f"Categories: {cats}",
                  j["url"], ""]
    return "\n".join(lines)
...
def main():
    ...
    if new:
        ...
        Path("latest_alert.md").write_text(format_alert(new))
    else:
        print("No new matching internships.")
        Path("latest_alert.md").write_text("# No new matching internships\n")
